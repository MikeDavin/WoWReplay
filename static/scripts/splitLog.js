...
{
    var selector = document.getElementById("saveDestination");
    ...
    var saveFight = function()
    {
        ...
        reader.onload = function(event)
        {
            if(event.target.error === null)
            {
                ...
                saveAs(new Blob([fightString],{type:"text/plain;charset=utf-8"}),
                    fights[fightIndex].encounter.getFileName().replace(':','')+".txt");
                fights[fightIndex].statusDiv.innerHTML = "Saved";
                fightIndex++;
                saveFight();
            }
            else
            {
                fights[fightIndex].statusDiv.innerHTML = "Error reading in fight: "+ event.target.error;
            }
        };
    ...
    var readNextBlock = function()
    {
        ...
        reader.onload = function(event)
        {
            ...
            if(offset >= file.size)
            {
                ...
                saveAllButton.innerHTML = '<span class="glyphicon glyphicon-download-alt"></span> Save All';
...
var FightChunk = function(roughStart, chunkOffset, roughEnd, firstLine, lastLine)
{
        ...
        fightObject.htmlButton.onclick = function()
        {
            ...
            reader.onload = function(event)
            {
                if(event.target.error === null)
                {
                    ...
                    var blob = new Blob([fightString],{type:"text/plain;charset=utf-8"});
                    saveAs(blob, fightObject.encounter.getFileName()+".txt"); 
                    fightObject.statusDiv.innerHTML = "Saved";
                }
                else
                {
    ...
    this.trimFight = function(fightString)
    {
        ...
        while(-1 !== (nextNewline = fightString.indexOf('\n', lastNewline)))
        {
            ...
            {
                foundTheData = true;
                //console.log("startFound, lastNewline: "+ lastNewline);
            }
            if(foundTheData)
            {
                result += line;
            ...
            {
                //console.log("end found, lastNewline: "+lastNewline);
                break;
            }
            lastNewline = nextNewline+1;
        }
        //console.log("trimFight "+this.fileName+" : result length "+result.length);
        return result;
    };
};
